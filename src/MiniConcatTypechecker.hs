...
preludeSrc = unlines
  ...
  , "## guard ladders as first-class words: each guard is [cond] [action],"
  , "## joined by if / elif and closed by otherwise.  first hit wins."
  , "##   x [c] [a] >> if  [d] [b] >> elif  [z] >> otherwise"
  , "## decision guards: c just decides (a router OR a bool — verdict"
  , "## collapses either to a Bool); the action runs on a kept copy of x."
  , "def if        = (x c a -> x >> c ... >> apply >> verdict >> (x >> a ... >> apply >> in1 | x >> in2) >> merge)"
  , "## run `if` again on the still-unanswered (miss) track"
  , "def elif      = (s c a -> s >> (in1 | (m -> m c a >> if)) >> merge)"
  , "## close the ladder with a total default (no condition)"
